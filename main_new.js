...
// 1、获取code码
async function getHuaMiCode(params) {
  ...
  try {
    ...
    if (!code) throw new Error('第一步：解析code码失败！！！')
    console.log('第一步：获取code码成功：', '虚拟IP:', virtualIp)
    // 10s 后再发送请求, 避免github问题
    await delay()
    getHuaMiToken(code, params)
  } catch (error) {
    // 重试3遍，每遍隔5s重试
    errGetAccessCode++
    if (errGetAccessCode <= 3) {
      await delay(5000)
      getHuaMiCode(params)
    } else {
      throw Error(
        '第一步：获取当前账号的【code】码失败-->重试3遍后还是失败：',
        virtualIp,
        error
      )
    }
...
// 2、获取token
async function getHuaMiToken(code, params) {
  ...
  try {
    ...
    if (!app_token) {
      throw Error('第二步：获取token失败！！！')
    }
    console.log('第二步：获取token成功：', '虚拟IP:', virtualIp)
    // 10s 后再发送请求, 避免github问题
    await delay()
    handleRunStep({ app_token, user_id, step: params.step })
  } catch (error) {
    // 重试3遍，每遍隔5s重试
    errGetTokenCode++
    if (errGetTokenCode <= 3) {
      await delay(5000)
      getHuaMiToken(code, params)
    } else {
      throw Error(
        '第二步：获取当前账号的【token】失败-->重试3遍后还是失败：',
        error
      )
    }
  }
...
async function handleRunStep(params) {
  ...
  try {
    ...
    if (errRunStepCode <= 3) {
      await delay(5000)
      handleRunStep(params)
    } else {
      throw Error('第三步：刷步数失败-->重试3遍后还是失败：', error)
    }
  }
}
